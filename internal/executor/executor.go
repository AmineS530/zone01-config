...
func ApplyConfig(cfg config.UserConfig, exportSettings bool, out io.Writer) error {
	...
	// 2. Clone the zone01-config repository
	destDir := filepath.Join(homeDir, "zone01-config")
	...
	cloneCmd := exec.Command("git", "clone", "--quiet", "--depth=1", "https://github.com/AmineS530/zone01-config.git", destDir)
	...
	// 10. Install 2nd instance of Discord if requested
	if cfg.EnableDiscord {
		logger.Info("Installing 2nd instance of Discord (Portable)...")
		discordScript := filepath.Join(destDir, "dis.zsh")
		discordCmd := exec.Command("bash", discordScript, "-p")
		if err := runCommandStream(discordCmd, out); err != nil {
			logger.Warning("Discord installation completed with warning: %v", err)
		} else {
			logger.Success("2nd instance of Discord Portable installed and pinned successfully.")
		}
	}
	...
	// 12. Export selections to $HOME/.config/zone01-config/config.json if requested
	if exportSettings {
		configDir := filepath.Join(homeDir, ".config", "zone01-config")
