...
type tuiModel struct {
	...
	enableDiscord    bool // true = Yes, false = No
...
func RunWizard() {
	...
	configFilePath := filepath.Join(homeDir, ".config", "zone01-config", "config.json")
	...
	if initCfg.BackgroundImage != "" && !strings.Contains(initCfg.BackgroundImage, "zone01-config") {
	...
	m := tuiModel{
		...
		enableDiscord:    initCfg.EnableDiscord,
...
func (m tuiModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case 0: // Import Saved settings prompt
			...
			case "enter":
				if m.importPrompt {
					...
					m.enableDiscord = m.cfg.EnableDiscord
			...
			if !m.applyBg {
				...
				if m.bgChoiceCursor == 2 {
					...
					case "enter":
						homeDir, _ := os.UserHomeDir()
						if m.bgChoiceCursor == 0 {
							m.cfg.BackgroundImage = filepath.Join(homeDir, "zone01-config", "wallpapers", "Background.jpeg")
						} else if m.bgChoiceCursor == 1 {
							selectedWP := m.repoWallpapers[m.wallpaperCursor]
							m.cfg.BackgroundImage = filepath.Join(homeDir, "zone01-config", "wallpapers", selectedWP)
						}
						if m.importedSettings {
							m.step = 6
						} else {
		...
		case 5: // Discord and Zsh default selectors
			...
			case "left", "right", "h", "l", " ":
				if m.focusedInput == 0 {
					m.enableDiscord = !m.enableDiscord
				} else {
			...
			case "enter":
				...
				m.cfg.EnableDiscord = m.enableDiscord
...
func (m tuiModel) View() string {
	...
	s.WriteString(titleStyle.Render("   ZONE01 CONFIG - TERMINAL UI WIZARD  ") + "\n\n")
	...
	case 0: // Import Settings Prompt
		stepContent = fmt.Sprintf(
			" Loaded existing configurations in ~/.config/zone01-config/config.json.\n\n"+
	...
	case 4: // Background Setup
		if !m.applyBg {
			...
			wpSelector := ""
			if m.bgChoiceCursor == 1 && len(m.repoWallpapers) > 0 {
	...
	case 5: // Discord and Zsh defaults
		stepContent = fmt.Sprintf(
			" [Step 5/6] Environments & defaults\n\n"+
				"   %s Install 2nd Discord Portable?%s\n"+
				"     %s\n\n"+
				"   %s Set Zsh as Default Shell?%s\n"+
				"     %s\n\n"+
				"   %s",
			renderActiveLabel("[Option A]", m.focusedInput == 0), utils.Reset,
			renderHorizontalOptions([]string{"Yes", "No"}, getYesNoIndex(!m.enableDiscord)),
	...
	case 6: // Final Summary Checklist & Save
		...
		summary.WriteString(renderSummaryRow("Install Discord Portable", m.enableDiscord) + "\n")
		...
		if !m.finished {
			...
			if m.err != nil {
				...
				if !m.exportDone {
					...
					logs.WriteString(fmt.Sprintf("   %s✔ Finished successfully!%s\n\n", utils.Green, utils.Reset))
					if m.exportPrompt {
						logs.WriteString(fmt.Sprintf("   %s★ Settings exported successfully to ~/.config/zone01-config/config.json%s\n\n", utils.Green, utils.Reset))
					} else {
